...
Implementation details — source harvesting, Atlas retrieval, separate judgments, checks, and the citation graph — are in [IMPLEMENTATION.md](IMPLEMENTATION.md).
...
- **Relationships can be overread.** Shared authorship, advice, or citations invite closer inspection; they do not establish common data, invalid reasoning, bias, or falsity. (see the Perplexity comparison for a worked example — [evidence/perplexity/comparison.md](evidence/perplexity/comparison.md))
